...
export class CrossChainTestHarness {
	...
	constructor(
		...
		public inbox: GetContractReturnType<
			typeof InboxAbi,
			WalletClient<HttpTransport, Chain, Account>
		>,
		/** Message Bridge Outbox. */
		public outbox: GetContractReturnType<
			typeof OutboxAbi,
			WalletClient<HttpTransport, Chain, Account>
		>,
	...
	async mintTokensOnL1(amount: bigint) {
		...
		const txHash = await this.underlyingERC20.write.mint(
	...
	async sendTokensToPortalPublic(bridgeAmount: bigint, secretHash: Fr) {
		...
		const txHash2 = await this.tokenPortal.write.depositToAztecPublic(
			args,
			{} as any
		);
	...
	async sendTokensToPortalPrivate(
		...
		const txHash2 = await this.tokenPortal.write.depositToAztecPrivate(
			args,
			{} as any
		);
		await this.publicClient.waitForTransactionReceipt({ hash: txHash2 });
	...
	async mintTokensPrivateOnL2(amount: bigint, secretHash: Fr) {
		const receipt = await this.l2Token.methods
	...
	async consumeMessageOnAztecAndMintPublicly(bridgeAmount: bigint, secret: Fr) {
		...
		await this.l2Bridge.methods
			.claim_public(this.ownerAddress, bridgeAmount, secret)
			.send()
			.wait();
	}
	...
	async withdrawFundsFromBridgeOnL1(
		...
		expect(
			await this.outbox.read.hasMessageBeenConsumedAtBlockAndIndex(
				[BigInt(blockNumber), BigInt(messageIndex)],
				{}
			)
		).toBe(false);
