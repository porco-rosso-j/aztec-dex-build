...
export const uniswapL1L2TestSuite = (
	...
	describe("uniswap_trade_on_l1_from_l2", () => {
		...
		const WETH9_ADDRESS: EthAddress = EthAddress.fromString(
		...
		beforeAll(async () => {
			...
			const wethBalance = await wethCrossChainHarness.getL1BalanceOf(
				ownerEthAddress
			);
		...
		it("should uniswap trade on L1 from L2 funds privately (swaps WETH -> DAI)", async () => {
			...
			const [secretForMintingWeth, secretHashForMintingWeth] =
				wethCrossChainHarness.generateClaimSecret();
			const [secretForRedeemingWeth, secretHashForRedeemingWeth] =
				wethCrossChainHarness.generateClaimSecret();

			const tokenDepositMsgHash =
			...
			await wethCrossChainHarness.redeemShieldPrivatelyOnL2(
				wethAmountToBridge,
			...
			const nonceForBurnApprovalBeforeSwap = await uniswapL2Contract.methods
			...
			const withdrawContent = sha256ToField([
				Buffer.from(
					getFunctionSelector("withdraw(address,uint256,address)").substring(2),
					"hex"
				),
			...
			const nonceForBurnApprovalAfterSwap = await uniswapL2Contract.methods
				.nonce_for_burn_approval()
				.view();
			expect(nonceForBurnApprovalAfterSwap).toBe(
				nonceForBurnApprovalBeforeSwap + 1n
			);
			...
			const [swapPrivateL2MessageIndex, swapPrivateSiblingPath] =
			...
			const daiL1BalanceOfPortalAfter =
				await daiCrossChainHarness.getL1BalanceOf(
					daiCrossChainHarness.tokenPortalAddress
				);
		...
		it("should uniswap trade on L1 from L2 funds publicly (swaps WETH -> DAI)", async () => {
			...
			// Store balances
			const wethL2BalanceBeforeSwap =
				await wethCrossChainHarness.getL2PublicBalanceOf(ownerAddress);
			const daiL2BalanceBeforeSwap =
			...
			await ownerWallet
				.setPublicAuthWit(transferMessageHash, true)
				.send()
				.wait();
		...
		it("can't swap if user passes a token different to what the bridge tracks", async () => {
			...
			const [secretForRedeemingWeth, secretHashForRedeemingWeth] =
				wethCrossChainHarness.generateClaimSecret();
			await wethCrossChainHarness.mintTokensPrivateOnL2(
				wethAmountToBridge,
				secretHashForRedeemingWeth
			);
			await wethCrossChainHarness.redeemShieldPrivatelyOnL2(
			...
			await expect(
				...
				"Assertion failed: input_asset address is not the same as seen in the bridge contract"
			);
		});
		...
		it("I don't need approval to call swap_public if I'm swapping on my own behalf", async () => {
			...
			const [_, secretHashForDepositingSwappedDai] =
				daiCrossChainHarness.generateClaimSecret();
		...
		it("can't call swap_public on L1 if called swap_private on L2", async () => {
			...
			// Swap
			logger("Withdrawing weth to L1 and sending message to swap to dai");
			const [, secretHashForRedeemingDai] =
				daiCrossChainHarness.generateClaimSecret();

			const [, secretHashForDepositingSwappedDai] =
				daiCrossChainHarness.generateClaimSecret();
		...
		it("can't call swap_private on L1 if called swap_public on L2", async () => {
			...
			const [swapPublicL2MessageIndex, swapPublicSiblingPath] =
				await aztecNode.getL2ToL1MessageMembershipWitness(
					withdrawReceipt.blockNumber!,
					swapPublicLeaf
				);
