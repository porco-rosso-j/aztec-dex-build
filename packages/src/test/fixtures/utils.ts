...
import {
	...
	getCanonicalGasTokenAddress,
} from "@aztec/protocol-contracts/gas-token";
import {
	PXEService,
...
const getACVMConfig = async (logger: DebugLogger) => {
	try {
		...
		const tempWorkingDirectory = `${TEMP_DIR}/${randomBytes(4).toString(
			"hex"
		)}`;
		const acvmWorkingDirectory = ACVM_WORKING_DIRECTORY
			? ACVM_WORKING_DIRECTORY
			: `${tempWorkingDirectory}/acvm`;
		await fs.mkdir(acvmWorkingDirectory, { recursive: true });
		logger(
			`Using native ACVM binary at ${expectedAcvmPath} with working directory ${acvmWorkingDirectory}`
		);
		return {
			acvmWorkingDirectory,
			expectedAcvmPath,
			directoryToCleanup: ACVM_WORKING_DIRECTORY
				? undefined
...
async function initGasBridge({
	...
	await gasPortal.write.initialize(
		[
			...
			getCanonicalGasTokenAddress(
				l1ContractAddresses.gasPortalAddress
			).toString(),
		],
		{} as any
	);
...
async function setupWithRemoteEnvironment(
	...
	logger(
		"PXE created, constructing available wallets from already registered accounts..."
	);
	...
	if (["1", "true"].includes(ENABLE_GAS)) {
		...
		await deployCanonicalGasToken(
			new SignerlessWallet(pxeClient, new DefaultMultiCallEntrypoint())
		);
	}
...
export async function setup(
	...
	if (PXE_URL) {
		...
		return await setupWithRemoteEnvironment(
			hdAccount,
			config,
			logger,
			numberOfAccounts
	...
	const deployL1ContractsValues =
		...
		(await setupL1Contracts(config.rpcUrl, hdAccount, logger));
	...
	const { pxe, accounts, wallets } = await setupPXEService(
		numberOfAccounts,
		aztecNode!,
		pxeOpts,
		logger
	...
	const teardown = async () => {
		...
		if (acvmConfig?.directoryToCleanup) {
			...
			logger(
				`Cleaning up ACVM temp directory ${acvmConfig.directoryToCleanup}`
			);
			await fs.rm(acvmConfig.directoryToCleanup, {
				recursive: true,
				force: true,
			});
		}
...
export async function publicDeployAccounts(
	...
	const accountAddressesToDeploy = accountsToDeploy.map((a) =>
		"address" in a ? a.address : a
	);
...
function getJobName() {
	return (
		...
		expect.getState().currentTestName?.split(" ")[0].replaceAll("/", "_") ??
		"unknown"
	);
}
...
export function getLogger() {
	const describeBlockName = expect
		.getState()
		.currentTestName?.split(" ")[0]
		.replaceAll("/", ":");
	if (!describeBlockName) {
		const name =
			expect.getState().testPath?.split("/").pop()?.split(".")[0] ?? "unknown";
...
export const expectsNumOfEncryptedLogsInTheLastBlockToBe = async (
	...
	const encryptedLogs = await aztecNode.getLogs(
		l2BlockNum,
		1,
		LogType.ENCRYPTED
	);
 ...
 */
export const expectUnencryptedLogsInTxToBe = async (
	tx: SentTx,
	logMessages: string[]
) => {
	const unencryptedLogs = (await tx.getUnencryptedLogs()).logs;
	const asciiLogs = unencryptedLogs.map((extendedLog) =>
		extendedLog.log.data.toString("ascii")
	);
...
export function getBalancesFn(
	...
	const balances = async (...addresses: AztecAddress[]) => {
		const b = await Promise.all(
			addresses.map((address) => method(address).view())
		);
		const debugString = `${symbol} balances: ${addresses
			.map((address, i) => `${address}: ${b[i]}`)
			.join(", ")}`;
		logger(debugString);
		return b;
	};
...
export async function deployCanonicalGasToken(deployer: Wallet) {
	...
	if (
		await deployer.isContractClassPubliclyRegistered(
			canonicalGasToken.contractClass.id
		)
	) {
		return;
	}
	...
	await new BatchCall(deployer, [
		(
			await registerContractClass(deployer, canonicalGasToken.artifact)
		).request(),
	...
	await expect(
		deployer.isContractClassPubliclyRegistered(
			canonicalGasToken.contractClass.id
		)
	).resolves.toBe(true);
	await expect(
		deployer.getContractInstance(canonicalGasToken.instance.address)
